 ...
 * @type {symbol}
 */
export const flagSymbol = Symbol( 'd64 async function flag( flags ):object' )
export const flag = Object.defineProperty( d64, flagSymbol, {
    ...
    value: async function flag( flags ){
        ...
        function flagsChecker( flagsRipped ) {
            ...
            if ( ( objKeys.includes( '--file' ) || objKeys.includes( '-f' ) ) && ( objKeys.includes( '--string' ) || objKeys.includes( '-s' ) ) ) {
            ...
            if ( ( objKeys.includes( '--json' ) || objKeys.includes( '-j' ) )  && ( objKeys.includes( '--stdout' ) || objKeys.includes( '-p' ) ) ) {
         ...
         * @returns {string[]}
         */
        async function emptyFinder( flags ) {
        
            for ( const key in flags ) {
                if ( await undefined_( flags[ parseInt( key ) + 1 ] ) === false ) {
                    if ( flags[ key ].search( '[^--][a-z]*' ) !== 0 && flags[ parseInt( key ) + 1 ].search( '[^--][a-z]*' ) !== 0 )
