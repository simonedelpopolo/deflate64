...
import { array_, boolean_, null_ } from 'oftypes'
import { compression, compressionSymbol, help, helpSymbol, remote, remoteSymbol } from './exporter.js'
 ...
 *
 * @param {string} message - The message to the stderr.
 */
 ...
 *
 * @param {string} message - The message to the stdout.
 */
    ...
    for ( const flag in flags ) {
        ...
        switch ( flag ) {
            ...
            case '--file':
                ...
                if ( flagsObjectKeys.includes( '--remote' ) === true || flagsObjectKeys.includes( '-r' ) === true )
                    continue
            ...
            case '--stdout':
                ...
                if ( flagsObjectKeys.includes( '--json' ) === true || flagsObjectKeys.includes( '-j' ) === true )
                    continue
                
                /**
                 * If you do not save and not stdout the result, why you should use it?
                 */
                if ( flagsObjectKeys.includes( '--save' ) === false && flagsObjectKeys.includes( '-o' ) === false && await true_false( flags[ flag ] ) === false ) {
            ...
            case '--json': {
                ...
                if ( boolean_( jsonCheck ) !== true ) {
                    ...
                    const isJson = await is_json( jsonValue )
                    if( isJson !== true ) {
                        console.log( `check the json string, something doesn't work -> ${jsonValue}` )
                        process.exit( 1 )
                    }
                }
                command.output.out.print = await true_false( flags[ flag ] )
     ...
     */
        ...
        switch( command.input.type ){
            ...
            case 'string':
                ...
                break
            case 'remote':
                stringObject = await remote[ remoteSymbol ]( command.input.remote )
                break
        }
        
        if ( command.input.object === true ) {
            stringObject = await inObject( command.input.string )
            if( typeof stringObject.string === 'undefined' ) {
                stderr( stringObject )
                process.exit( 1 )
            }
